...
export default function NovaEncomenda({ artigos, setEcraAtual, carregarDados, mostrarAlerta }: Props) {
  ...
  const analisarTextoInteligente = (texto: string) => {
    ...
    // Procurar "Cliente: FC Pedroso"
    const matchCliente = texto.match(/Cliente[:\s]*(.+)/i);
    if (matchCliente) clientePossivel = matchCliente[1].trim();

    // Somar as quantidades (Linhas que começam por número e têm "TAM")
    linhas.forEach(linha => {
      const matchQtdLinha = linha.match(/^(\d+)\s+TAM/i);
      if (matchQtdLinha) somaQuantidades += parseInt(matchQtdLinha[1]);
    });
    ...
    if (somaQuantidades > 0) {
      ...
      mostrarAlerta(
        'Leitura Concluída', 
        `Foram detetados a OP "${possivelOP || 'Desconhecida'}", o Cliente "${clientePossivel || 'Desconhecido'}" e um total de ${somaQuantidades} peças na tabela.\n\nPor favor, escolha o Artigo correspondente na lista abaixo e clique em Adicionar.`, 
        'sucesso'
      );
    } else {
      mostrarAlerta('Aviso', 'A OP e o Cliente podem ter sido lidos, mas não detetámos as quantidades da tabela. Verifique a nitidez da foto.', 'aviso');
    }
  };
