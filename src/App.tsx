...
export default function App() {
  ...
  const processarLeituraScanner = (codigosLidos: any[]) => {
    ...
    const codigoQR = codigosLidos[0].rawValue; setPausarCamara(true);
    const artigo = artigos.find(a => a.codigo === codigoQR);
    if (artigo) { setArtigoSelecionado(artigo); setEcraAtual('formulario_saida'); } 
    else { mostrarAlerta('Artigo não encontrado', `Código "${codigoQR}" não existe.`, 'aviso'); setTimeout(() => setPausarCamara(false), 2000); }
  };
  ...
  const adicionarAoLote = (e: React.FormEvent) => {
    ...
    const novoItem: ItemExpedicao = { artigo_codigo: artigoSelecionado.codigo, artigo_nome: artigoSelecionado.nome, quantidade: qtdNum, total_faturado: qtdNum * artigoSelecionado.preco, op_numero: opAUsar, tamanho: formTamanho };
    setListaExpedicao([...listaExpedicao, novoItem]); setFormQtd(''); setArtigoSelecionado(null); setEcraAtual('resumo_expedicao');
  };
          ...
          <div style={{ animation: 'fadeIn 0.3s' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div style={{ backgroundColor: 'var(--surface-color)', padding: '20px', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
                <h3 style={{ margin: '0 0 10px 0', color: 'var(--primary-color)' }}>🎯 Expedição por OP (Inteligente)</h3><p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '15px' }}>O sistema ajuda a controlar as quantidades exatas que faltam entregar.</p>
                <select value={opSelecionada} onChange={e => setOpSelecionada(e.target.value)} style={{...inputStyle, marginBottom: '15px'}}><option value="" disabled>Selecione a OP pendente...</option>{obterOportunidadesPendentes().map(op => <option key={op} value={op}>{op}</option>)}</select>
              ...
              <div style={{ backgroundColor: 'var(--surface-color)', padding: '20px', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
                <h3 style={{ margin: '0 0 10px 0' }}>🔓 Expedição Livre (Manual)</h3><p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '15px' }}>Envio direto sem controlo de encomendas prévias.</p>
            ...
            {modoExpedicao === 'op' && (
              <div style={{ backgroundColor: 'var(--surface-color)', padding: '15px', borderRadius: '12px', marginBottom: '25px', border: '1px solid var(--border-color)' }}>
                ...
                {encomendas.filter(e => e.op_numero === opSelecionada).map(enc => {
                  const qtdLida = listaExpedicao.filter(l => l.artigo_codigo === enc.artigo_codigo).reduce((sum, curr) => sum + curr.quantidade, 0); const concluido = qtdLida >= enc.quantidade_pedida;
                  return (
                    <div key={enc.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px dashed rgba(255,255,255,0.05)' }}>
                      <span style={{ color: concluido ? '#22c55e' : 'var(--text-primary)', textDecoration: concluido ? 'line-through' : 'none' }}>{enc.artigo_nome}</span><strong style={{ color: concluido ? '#22c55e' : '#ef4444' }}>{qtdLida} / {enc.quantidade_pedida} {concluido ? '✅' : ''}</strong>
                    </div>
                  );
                })}
              </div>
